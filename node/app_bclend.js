...
const insight = new explorers.Insight(properties.insightBitcoinUrl, properties.network);
...
app.post('/address_gen', function(req, res){
	console.log('POST REQUEST FOR GENERATING ETHER ADDRESS AND BITCOIN ADDRESS');
...
app.post('/ether_transfer', function(req, res){
   console.log('POST /');
    console.log();
    res.writeHead(200, {'Content-Type': 'application/json'});
	...
	var estimate = web3.eth.estimateGas({from: from, to: to, amount:value}).then(function (limit) {
		...
		var balance = value - (gasprice.times(limit));
		var valueToHex = web3.utils.toHex(balance);
...
app.post('/bitcoin_transfer', function(req, res) {

            var body = req.body;
            
            console.log('\nBitcoin transfer request FROM - ' + body.fromAddress + ' , TO - ' + bitcoinToaddress);
            ...
            insight.getUnspentUtxos(bitcoinFromaddress, function(error, utxos) {

			try{
			
                var fromAddress = new Address(bitcoinFromaddress);
                var toAddress = new Address(bitcoinToaddress);
                if (error) {
                    // any other error
					result.code=100;
                    ...
                    if (utxos.length == 0) {
                        // if no transactions have happened, there is no balance
						// on the address.
						result.code=100;
						result.description ="There is no balance on the address";
						result.errorCode=140;
						// errorBody.errorDesc = "You don't have enough utox
						// Satoshis to cover the miner fee.";
						res.end(JSON.stringify(result));
                    }
					...
					// get balance
					if(balance<=0 || readBalance <=0){
						result.code=100;
						result.description ="There is no balance on the address";
						result.errorCode=150;
						
					// errorBody.errorDesc = "You don't have enough balance.";
						res.end(JSON.stringify(result));
					...
					// errorBody.errorDesc = "";
						...
						if (balance > 0 && balance - minerFee > 0) {
							balance = balance - minerFee;
							...
							// errorBody.errorDesc = "You don't have enough
							// Satoshis to cover the miner fee.";
							res.end(JSON.stringify(result));
						}
						...
						let bitcore_transaction = new bitcore.Transaction()
															.from(utxos)
															.to(bitcoinToaddress, transactionAmount)
															.fee(minerFee)
															.change(bitcoinFromaddress) 
															.sign(bitcoinPrivatekey);
				 ...
				 * res.end(JSON.stringify(errorBody)); }else if (balance <= 0){
